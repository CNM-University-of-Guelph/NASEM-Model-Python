...
def calculate_InfRum_RUPIn(InfRum_CPAIn, InfRum_CPBIn, InfRum_CPCIn, InfRum_NPNCPIn, Inf_KdCPB, coeff_dict):
    ...
    InfRum_RUPIn = (InfRum_CPAIn - InfRum_NPNCPIn) * coeff_dict['fCPAdu'] + InfRum_CPBIn * coeff_dict['KpConc'] / (Inf_KdCPB + coeff_dict['KpConc']) + InfRum_CPCIn     # Line 1084
    return InfRum_RUPIn
...
def calculate_InfSI_idCPIn(InfSI_idTPIn, InfSI_NPNCPIn, coeff_dict):
    req_coeff = ['dcNPNCP']
    check_coeffs_in_coeff_dict(coeff_dict, req_coeff)
    InfSI_idCPIn = InfSI_idTPIn + InfSI_NPNCPIn * coeff_dict['dcNPNCP'] / 100  #SI infused idTP + urea or ammonia, Line 1092
    return InfSI_idCPIn

def calculate_Inf_idCPIn(InfRum_idRUPIn, InfSI_idCPIn):
    Inf_idCPIn = InfRum_idRUPIn + InfSI_idCPIn  #RUP + intestinally infused, Line 1093
    return Inf_idCPIn
...
def calculate_Inf_DigFAIn(Inf_FAIn, coeff_dict):
    ...
    check_coeffs_in_coeff_dict(coeff_dict, req_coeff)
    Inf_DigFAIn = Inf_FAIn * coeff_dict['TT_dcFA_Base']	#Line 1306, used dcFA which is similar to oil, but should define for each infusate
    return Inf_DigFAIn
...
def calculate_infusion_data(infusion_input, Dt_DMIn, coeff_dict):
    ...
    for name in infused_CP_fraction:
        infusion_data[f"{name}In"] = infusion_input['Inf_CP_g'] / 1000 * infusion_input[f"{name}Rum_CP"] / 100
    ...
    for name in infused_DM_list:
        infusion_data[f"{name}"] = infusion_data[f"{name}In"] / (Dt_DMIn + infusion_data['Inf_DMIn']) * 100
    # Line 860-871
    ...
    for name in infused_Rum:
        infusion_data[f"InfRum_{name}In"] = infusion_data['Inf_Rum'] * infusion_data[f"Inf_{name}In"]   # Line 880-897
    ...
    for name in infused_SI:
        infusion_data[f"InfSI_{name}In"] = infusion_data['Inf_SI'] * infusion_data[f"Inf_{name}In"] # Line 900-914
    ...
    for name in infused_Art:
        infusion_data[f"InfArt_{name}In"] = infusion_data['Inf_Art'] + infusion_data[f"Inf_{name}In"]   # Line 917-931
