...
def calculate_Man_out(An_StatePhys: str, An_DMIn: float, Dt_K: float) -> float:
    ...
    """
    Man_out = -28.3 + 3.6 * An_DMIn + 12.4 * Dt_K if An_StatePhys != "Calf" else None   # kg wet manure/d, Line 3246-3247
    return Man_out
...
def calculate_Man_VolSld(Dt_DMIn: float, InfRum_DMIn: float, InfSI_DMIn: float, An_NDF: float, An_CP: float) -> float:
    """
    ...
    Man_VolSld = 0.364 * (Dt_DMIn + InfRum_DMIn + InfSI_DMIn) + 0.026 * An_NDF - 0.078 * An_CP  # Line 3251
    return Man_VolSld


def calculate_Man_VolSld2(Fe_OM: float, Dt_LgIn: float, Ur_Nout_g: float) -> float:
    """
    Man_VolSld: Volatile solids in manure (kg/d)
    """
...
def calculate_Man_Nout_g(Ur_Nout_g: float, Fe_N_g: float, Scrf_N_g: float) -> float:
    """
    Man_Nout_g: Manure nitrogen by summation of fecal, urinary, and scurf predictions (g/d)
    """
    Man_Nout_g = Ur_Nout_g + Fe_N_g + Scrf_N_g  # g/d; direct by summation of fecal, urinary, and scurf predictions, Line 3257
    return Man_Nout_g
...
def calculate_Man_Nout2_g(An_NIn_g: float, An_Nprod_g: float) -> float:
    ...
    """
    Man_Nout2_g = An_NIn_g - An_Nprod_g # g/d; by difference from intake and productive NP use, Line 3258
    return Man_Nout2_g
...
def calculate_Man_MacMin_out(Man_Ca_out: float, Man_P_out: float, Man_Mg_out: float, Man_K_out: float, Man_Na_out: float, Man_Cl_out: float) -> float:
    """
    Man_MacMin_out: Macrominerals in manure (g/d)
    """
    Man_MacMin_out = Man_Ca_out + Man_P_out + Man_Mg_out + Man_K_out + Man_Na_out + Man_Cl_out  # Line 3268
...
def calculate_Man_MicMin_out(Man_Cu_out: float, Man_Fe_out: float, Man_Mn_out: float, Man_Zn_out: float) -> float:
    """
    Man_MicMin_out: Microminerals in manure (g/d)
    """
    Man_MicMin_out = Man_Cu_out + Man_Fe_out + Man_Mn_out + Man_Zn_out # Not a complete accounting. Many other micros also present, Line 3276
    return Man_MicMin_out


def calculate_Man_Min_out_g(Man_MacMin_out: float, Man_MicMin_out: float) -> float:
    """
    Man_Min_out_g: Minerals in manure (g/d)
    """
    Man_Min_out_g = Man_MacMin_out + Man_MicMin_out / 1000  # also an incomplete summation as many micros are not represented, Line 3278
    return Man_Min_out_g


def calculate_Man_Wa_out(An_StatePhys: str, Man_out: float, Fe_OM: float, Ur_Nout_g: float, Man_Min_out_g: float) -> float:
    """
    ...
    if An_StatePhys == "Calf":  # Line 3336
        ...
        Man_Wa_out = Man_out - Fe_OM - Ur_Nout_g / 0.45 / 1000 - Man_Min_out_g / 1000 # Manure Water estimate, L/d, Line 3332
    return Man_Wa_out
