...
def calculate_An_MPm_g_Trg(Fe_MPendUse_g_Trg: float, Scrf_MPuse_g_Trg: float, Ur_MPendUse_g: float) -> float:
    """
...
def calculate_Body_MPUse_g_Trg_initial(Body_NPgain_g: float, coeff_dict: dict) -> float:
    """
    ...
    Body_MPUse_g_Trg = Body_NPgain_g / coeff_dict['Kg_MP_NP_Trg']   # Line 2675, kg MP/d for NP gain    
...
def calculate_Mlk_MPUse_g_Trg(Trg_Mlk_NP_g: float, coeff_dict: dict) -> float:
    ...
    check_coeffs_in_coeff_dict(coeff_dict, req_coeff)
    Mlk_MPUse_g_Trg = Trg_Mlk_NP_g / coeff_dict['Kl_MP_NP_Trg'] # kg MP/d for target milk protein lactation, Line 2677
    return Mlk_MPUse_g_Trg


def calculate_An_MPuse_g_Trg_initial(An_MPm_g_Trg: float, Body_MPUse_g_Trg: float, Gest_MPUse_g_Trg: float, Mlk_MPUse_g_Trg: float) -> float:
    """
...
def calculate_Min_MPuse_g(An_StatePhys: str, An_MPuse_g_Trg: float, An_BW: float, An_BW_mature: float, An_MEIn_approx: float) -> float:
    """
    Min_MPuse_g: I think minimum MP use, g/d, but not 100% sure
    """
    if An_StatePhys == "Heifer" and An_MPuse_g_Trg < ((53 - 25 * An_BW / An_BW_mature) * An_MEIn_approx):   # Line 2686-2687
...
def calculate_Frm_MPUse_g_Trg(An_StatePhys: str, Frm_NPgain_g: float, Kg_MP_NP_Trg: float, Diff_MPuse_g: float) -> float:
    """
    Frm_MPUse_g_Trg: kg MP/d for Frame NP gain
    """
    Frm_MPUse_g_Trg = Frm_NPgain_g / Kg_MP_NP_Trg   # kg MP/d for Frame NP gain, Line 2674
...
def calculate_Kg_MP_NP_Trg(An_StatePhys: str, An_Parity_rl: int, An_BW: float, An_BW_empty: float, An_BW_mature: float, An_BWmature_empty: float, MP_NP_efficiency_dict: dict, coeff_dict: dict) -> float:
    """
    Kg_MP_NP_Trg: Conversion of NP to MP for growth
    """
    req_coeff = ['Body_NP_CP']
    check_coeffs_in_coeff_dict(coeff_dict, req_coeff)
    Kg_MP_NP_Trg = 0.60 * coeff_dict['Body_NP_CP']  #Default value for Growth MP to TP.  Shouldn't be used for any., Line 2659
    condition = (An_Parity_rl == 0) & (An_BW_empty / An_BWmature_empty > 0.12)
    Kg_MP_NP_Trg = np.where(condition,                                                  # Line 2660-2662
                            (0.64 - 0.3 * An_BW_empty / An_BWmature_empty) * coeff_dict['Body_NP_CP'],    # for heifers from 12% until 83% of BW_mature
                            Kg_MP_NP_Trg)
    Kg_MP_NP_Trg = np.where(Kg_MP_NP_Trg < 0.394 * coeff_dict['Body_NP_CP'],
                            0.394 * coeff_dict['Body_NP_CP'] * coeff_dict['Body_NP_CP'],    # Trap heifer values less than 0.39 MP to CP, Line 2663
                            Kg_MP_NP_Trg)
    Kg_MP_NP_Trg = np.where(An_StatePhys=="Calf",
                            (0.70 - 0.532 * (An_BW / An_BW_mature)) * coeff_dict['Body_NP_CP'],   # calves, Line 2664
                            Kg_MP_NP_Trg)
    Kg_MP_NP_Trg = np.where(An_Parity_rl > 0,
...
def calculate_Rsrv_MPUse_g_Trg(An_StatePhys: str, Diff_MPuse_g: float, Rsrv_NPgain_g: float, Kg_MP_NP_Trg: float) -> float:
    """
    ...
    if An_StatePhys == "Heifer" and Diff_MPuse_g > 0:
        ...
        Rsrv_MPUse_g_Trg = Rsrv_NPgain_g / Kg_MP_NP_Trg  # kg MP/d for Reserves NP gain, Line 2673
    return Rsrv_MPUse_g_Trg


def calculate_Body_MPUse_g_Trg(An_StatePhys: str, Diff_MPuse_g: float, Body_NPgain_g: float, Body_MPUse_g_Trg: float, Kg_MP_NP_Trg: float) -> float:
    """
...
def calculate_An_MPuse_g_Trg(An_MPm_g_Trg: float, Frm_MPUse_g_Trg: float, Rsrv_MPUse_g_Trg: float, Gest_MPUse_g_Trg: float, Mlk_MPUse_g_Trg: float) -> float:
    """
    An_MPuse_g_Trg: Metabolizable protein requirement (g/d)
    """
