...
def compare_values_with_tolerance(val1, val2, rtol=1e-3, atol=1e-2):
    if isinstance(val1, (list, np.ndarray, pd.Series)) and isinstance(val2, (list, np.ndarray, pd.Series)):
...
def compare_dicts_with_tolerance(dict1, dict2, rtol=1e-3, atol=1e-2):
    for key in dict1:
        if key in dict2:
            try:
                compare_values_with_tolerance(dict1[key], dict2[key], rtol=rtol, atol=atol)
            except AssertionError as e:
                print(f"Mismatch found for key '{key}': {dict1[key]} (expected) vs {dict2[key]} (actual)")
                raise e
        else:
...
def test_from_json_dict(nutrient_intake_dfs):
    for index, row in nutrient_intake_dfs.iterrows():
        ...
        if isinstance(row.Output, dict):
            ...
            pd.testing.assert_frame_equal(result, expected_output, rtol=1e-3, atol=1e-2)
